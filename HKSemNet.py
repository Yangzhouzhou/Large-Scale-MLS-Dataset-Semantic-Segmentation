...
class Network(nn.Module):
    ...
    def __init__(self, config):
        ...
        elif (config.name == 'HongKong'):
            self.class_weights = DP.get_class_weights('HongKong')
            self.fc0 = nn.Linear(7, 8)
            self.fc0_acti = nn.LeakyReLU()
            self.fc0_bath = nn.BatchNorm1d(8, eps=1e-6, momentum=0.99)
            nn.init.constant_(self.fc0_bath.weight, 1.0)
            nn.init.constant_(self.fc0_bath.bias, 0)
...
class IoUCalculator:
    ...
    def add_data(self, end_points):
        ...
        mat_label_list = np.arange(0, self.cfg.num_classes, 1) 
        conf_matrix = confusion_matrix(labels_valid, pred_valid, labels=mat_label_list)  
